...
pub fn initialize_mint(
    ...
    let mint_kp = if mint_kp.is_some() { mint_kp.unwrap() } else { Keypair::new() };
...
pub fn get_token_balance_or_zero(
    svm: &mut LiteSVM, 
    token_account: &Pubkey
) -> u64 {
    svm.get_account(token_account).map_or(0, |account| {
        let ata = Account::unpack(&account.data).map_err(|e| format!("Failed to unpack ata: {:?}", e));
...
pub fn transfer_tokens(
    ...
    let source_ata_pk = get_associated_token_address_with_program_id(
        &authority.pubkey(),
        mint, 
        &spl_token::id(), 
    );
    ...
    let source_delta = source_balance_before.checked_sub(source_balance_after).unwrap();
    let destination_delta = destination_balance_after.checked_sub(destination_balance_before).unwrap();
