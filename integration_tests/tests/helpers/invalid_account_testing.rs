...
impl<'a> InvalidAccountTestBuilder<'a> {
    ...
    fn execute_transaction(&mut self) -> litesvm::types::TransactionResult {
        ...
        // prepend CU budget increase to prevent compute budget exceeded errors
    ...
    fn assert_error(
        ...
        let logs = match tx_result {
            Ok(meta) => &meta.logs,
            Err(failed_meta) => &failed_meta.meta.logs,
        };
...
/// ```
///
/// With custom invalid account data:
/// ```rust
/// let custom_invalid_account = solana_sdk::account::Account {
///     lamports: 1_000_000_000,
///     data: vec![0; 100], // Custom invalid data
///     owner: Pubkey::new_unique(),
///     executable: false,
///     rent_epoch: 0,
/// };
