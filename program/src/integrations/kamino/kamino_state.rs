...
const FARMS_GLOBAL_CONFIG: usize = 2128;
const TREASURY_FEE_BFS_OFFSET: usize = 8 + 32;
pub struct FarmsGlobalConfig {
    pub treasury_fee_bps: u64,
}

impl <'a> TryFrom<&'a [u8]> for FarmsGlobalConfig {
    type Error = ProgramError;

    fn try_from(data: &'a [u8]) -> Result<Self, Self::Error> {
        if data.len() < 8 + FARMS_GLOBAL_CONFIG {
            return Err(ProgramError::InvalidAccountData)
        }
        ...
        let treasury_fee_bps = u64::from_le_bytes(
            data[TREASURY_FEE_BFS_OFFSET..TREASURY_FEE_BFS_OFFSET + 8]
                .try_into()
                .map_err(|_| ProgramError::InvalidAccountData)?
        );

        Ok(Self { treasury_fee_bps })
    }
}

impl FarmsGlobalConfig {
    const DISCRIMINATOR: [u8; 8] = FARMS_GLOBAL_CONFIG_DISCRIMINATOR;
}


const USER_FARM_STATE_SIZE: usize = 912;
const REWARDS_ISSUED_UNCLAIMED_OFFSET: usize = 8 + 8 + 32 + 32 + 1 + 7 + 160;
pub struct UserFarmState {
    pub rewards_issued_unclaimed: [u64; 10]
}

impl <'a> TryFrom<&'a [u8]> for UserFarmState {
    type Error = ProgramError;

    fn try_from(data: &'a [u8]) -> Result<Self, Self::Error> {
        if data.len() < 8 + USER_FARM_STATE_SIZE {
            return Err(ProgramError::InvalidAccountData)
        }
        ...
        let mut rewards_issued_unclaimed: [u64; 10] = [0; 10];
        let mut current_offset = REWARDS_ISSUED_UNCLAIMED_OFFSET;
        for slot in &mut rewards_issued_unclaimed {
            let reward = u64::from_le_bytes(
                data[current_offset..current_offset + 8]
                    .try_into()
                    .map_err(|_| ProgramError::InvalidAccountData)?
            );
            *slot = reward;
...
impl UserFarmState {
    ...
    pub fn get_rewards(&self, global_config: &AccountInfo, reward_index: usize) -> Result<u64, ProgramError> {
        let reward = self.rewards_issued_unclaimed[reward_index];
        if reward == 0 {
            return Ok(0)
        }

        let global_config_state = FarmsGlobalConfig::try_from(
            global_config.try_borrow_data()?.as_ref()
        )?;
...
mod tests {
    ...
    #[test]
    fn farms_global_config_try_from_works() {
        let treasury_fee_bps = 0;

        let raw = bs64.decode(RAW_FARMS_GLOBAL_CONFIG).expect("Invalid base 64 string");

        let global_config = FarmsGlobalConfig::try_from(raw.as_slice()).expect("FarmsGlobalConfig try from error");

        assert_eq!(global_config.treasury_fee_bps, treasury_fee_bps);
    }

    #[test]
    fn user_farm_state_try_from_works() {
        let rewards_issued_unclaimed = [
            3026768302, 0, 0, 0, 0, 0, 0, 0, 0, 0
        ];

        let raw = bs64.decode(RAW_USER_FARM_STATE).expect("Invalid base 64 string");

        let user_farm_state = UserFarmState::try_from(raw.as_slice()).expect("UserFarmState try from error");

        assert_eq!(user_farm_state.rewards_issued_unclaimed, rewards_issued_unclaimed);
    }
    ...
    const RAW_USER_FARM_STATE: &str = "";
    
    const RAW_FARMS_GLOBAL_CONFIG: &str = "";
}