...
pub fn derive_user_metadata_address(
    ...
    let (address, _) = try_find_program_address(&[b"user_meta", &user.as_ref()], &kamino_program)
        .ok_or(ProgramError::InvalidSeeds)?;
...
pub fn derive_market_authority_address(
    ...
    let (address, _) = try_find_program_address(&[b"lma", market.as_ref()], kamino_program)
        .ok_or(ProgramError::InvalidSeeds)?;
...
pub fn derive_farm_vaults_authority(
    ...
    let (address, _) =
        try_find_program_address(&[b"authority", farm_state.as_ref()], farms_program)
            .ok_or(ProgramError::InvalidSeeds)?;
