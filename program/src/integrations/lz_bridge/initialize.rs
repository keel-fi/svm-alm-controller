...
use crate::{
    ...
    integrations::lz_bridge::{config::LzBridgeConfig, lz_state::{OFTStore, PeerConfig, OFT_PEER_CONFIG_SEED}, state::LzBridgeState}, 
...
pub fn process_initialize_lz_bridge(
    ...
    let (expected_peer_config_pda, _bump) = SolanaPubkey::find_program_address(
        &[OFT_PEER_CONFIG_SEED, inner_ctx.oft_store.key().as_ref(), destination_eid.to_be_bytes().as_ref()],
        &SolanaPubkey::from(*inner_ctx.lz_program.key()),
    );
    if inner_ctx.peer_config.key().ne(&expected_peer_config_pda.to_bytes()) {
    ...
    let state = IntegrationState::LzBridge(
        LzBridgeState {
            _padding: [0u8;48]
        }
    );
