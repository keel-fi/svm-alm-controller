...
use crate::{error::SvmAlmControllerErrors, events::{ReserveUpdateEvent, SvmAlmControllerEvent}, instructions::ManageReserveArgs, state::{Controller, Permission, Reserve}};
...
pub fn process_manage_reserve(
    ...
    let ctx = ManageReserveAccounts::from_accounts(
        accounts
    )?;
   
    let args = ManageReserveArgs::try_from_slice(
        instruction_data
    ).unwrap();
