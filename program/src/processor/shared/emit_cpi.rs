...
pub fn emit_cpi<const N: usize>(
    ...
    let instruction_data = SvmAlmControllerInstruction::EmitEvent(EmitEventArgs {
    ...
    let instruction = Instruction {
        program_id: &crate::ID,
        accounts: &[AccountMeta::new(authority.key(), false, true)],
        data: instruction_data.as_slice(),
    };

    // Invoke the instruction with the PDA as a signer
    let signer = Signer::from(&signer_seeds);
    invoke_signed(&instruction, &[authority], &[signer])?;
