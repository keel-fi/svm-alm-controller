...
use pythnet_sdk::{
  accumulators::merkle::MerkleRoot, hashers::keccak256_160::Keccak160, messages::Message, pythnet::WORMHOLE_PID, wire::{
      from_slice,
      v1::{
          MerklePriceUpdate, WormholeMessage, WormholePayload
      },
  }
};
...
impl PythConfig {
  ...
  pub fn transform(
      &self,
      pyth_update: ValidatedPythUpdate,
  ) -> Result<(), ProgramError> {
  ...
  pub fn validate_update(
      ...
      let wormhole_message = WormholeMessage::try_from_bytes(vaa.payload().as_ref()).map_err(|_| {
      ...
      if !root.check(merkle_price_update.proof.clone(), &merkle_price_update.message.as_ref()) {
          log!("Invalid price update");
          return Err(ProgramError::InvalidAccountData.into());
      }

      let message = from_slice::<byteorder::BE, Message>(merkle_price_update.message.as_ref()).map_err(|_| {
          log!("Failed to deserialize message");
          ProgramError::InvalidAccountData
      })?;

      match message {
          Message::PriceFeedMessage(price_feed_message) => Ok(
              ValidatedPythUpdate {
...
fn deserialize_guardian_set_checked(
  ...
  let expected_address = find_program_address(
      &[GUARDIAN_SET_SEED_PREFIX, guardian_set.index.to_be_bytes().as_ref(),], wormhole
  ).0;
  if expected_address != *account_info.key() {
      log!("Invalid Guardian Set PDA");
      return Err(ProgramError::InvalidAccountData.into());
  }
  let timestamp = Clock::get().map_err(|_| ProgramError::UnsupportedSysvar)?.unix_timestamp as u32;
...
impl GuardianSetData {
  ...
  pub fn try_deserialize(data: &[u8]) -> Result<Self, ProgramError> {
      ...
      let creation_time = u32::from_le_bytes(data[offset..offset + 4].try_into().map_err(|_| {
      ...
      let expiration_time = u32::from_le_bytes(data[offset..offset + 4].try_into().map_err(|_| {
